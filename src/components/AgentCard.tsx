...
const BADGE: Record<AgentStatus, { label: string; color: "informative" | "success" | "danger" | "warning" | "brand" }> = {
  ...
  failed: { label: "Failed today", color: "danger" },
...
export function AgentCard({ agent }: { agent: Agent }) {
  ...
  const { statusOf, fireStates, fire, setOpenAgentId, demo } = useApp();
  const [firing, setFiring] = useState(false);
  const status = statusOf(agent);
  const fireState = fireStates.get(agent.id);
  const badge = BADGE[status];
  const canFire = !!agent.triggerUrl || demo;
  ...
  const onFire = async (e: React.MouseEvent) => {
    e.stopPropagation();
    setFiring(true);
    try {
      await fire(agent);
    } catch {
      /* surfaced via toast in AgentGrid-level handler; keep card resilient */
    } finally {
      setFiring(false);
    }
  };
        ...
        {canFire ? (
          <Tooltip content="Fire this agent now" relationship="label">
            <Button
              ...
              size="small"
              icon={<PlayFilled />}
              disabled={firing || status === "running"}
              onClick={onFire}
          ...
          <Tooltip
            content="Add a trigger URL (pda_triggerurl) to this agent's row to fire it from here"
            relationship="label"
          >
