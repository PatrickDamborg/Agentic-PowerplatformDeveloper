...
export function AgentDrawer() {
  ...
  const {
    agents, openAgentId, setOpenAgentId, statusOf, fireStates, fire, demo,
    approvals, respond
  } = useApp();
      ...
      <DrawerBody className={styles.body}>
        ...
        <div>
          <Button
            appearance="primary"
            icon={<PlayFilled />}
            disabled={firing || status === "running" || (!agent.triggerUrl && !demo)}
            onClick={onFire}
        ...
        <Divider />
        <div className={styles.section}>Recent runs ({agent.runs.length})</div>
