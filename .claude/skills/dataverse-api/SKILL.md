...
1. **Check in-repo skill docs first** — `model-driven-app/SKILL.md`, `data-model/`, and this file contain confirmed patterns from live API calls. Use the Microsoft Learn MCP only for operations not yet covered here.
...
4. **Web API only — no `pac`** — every operation this skill covers (data, metadata, solutions, security) goes through the Web API. The `pac` CLI is reserved solely for PCF project scaffolding and local dev workflows (`pac pcf init`, test harness, `pac pcf push`); never use it for anything in this file.
...
Always verify the record was persisted immediately after creation — some endpoints (e.g. `appmodules`) return a fake EntityId on silent failure. See `model-driven-app/SKILL.md` for details.
...
Entity components (componenttype=1) cannot be added via `AddAppComponents` — all known formats silently fail or error. See `model-driven-app/SKILL.md` Known Gotchas #5.
