...
## `claude-in-chrome` vs `playwright-cli`

This skill covers `claude-in-chrome`: interactive, visual verification in an existing Chrome session — checking live record state, PCF rendering, and one-off UI spot-checks like the ones below.

For scripted, repeatable browser checks or authoring Playwright tests, use the installed **`playwright-cli`** skill instead — it's the right tool when the check needs to run more than once or become part of a test suite.
