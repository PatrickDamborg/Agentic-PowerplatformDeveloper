<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_apikey`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_apikey -->

# PumApikey (`pum_apikey`)

Read-shape from the Web API: lookups appear as `_<name>_value`; write them via `@odata.bind`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `pum_name` | String | ✓ | **NAME**; max length 250 |
| `_ownerid_value` | Owner | ✓ | Lookup → systemuser, team; write `OwnerId@odata.bind` = `/systemusers({id})`, `/teams({id})` |
| `pum_apikeyid` | Uniqueidentifier | ✓ |  |
| `pum_systemmessage` | String | ✓ | max length 4000 |
| `statecode` | State | ✓ |  |
...
| `pum_aiprovider` | Picklist |  |  |
| `pum_apiendpoint` | String |  | max length 500 |
| `pum_firstmessage` | String |  | max length 2000 |
| `pum_gptmodel` | String |  | max length 100 |
| `pum_key` | String |  | max length 250 |
| `pum_structuredoutput` | Boolean |  |  |
| `pum_temperature` | Decimal |  | range 0..1; 2 decimals |
| `pum_usermessage` | String |  | max length 100 |
| `statuscode` | Status |  |  |
...
### `pum_aiprovider` choices

| Value | Label |
|---|---|
| 493840000 | OpenAI |
| 493840001 | Azure OpenAI |
