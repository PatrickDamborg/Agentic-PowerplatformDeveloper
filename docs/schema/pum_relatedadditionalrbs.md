<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_relatedadditionalrbs`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_relatedadditionalrbs -->

# PumRelatedadditionalrbs (`pum_relatedadditionalrbs`)
...
| `pum_relatedadditionalrbsid` | Uniqueidentifier | ✓ |  |
| `statecode` | State | ✓ |  |
...
| `_pum_additionalrbslookup_value` | Lookup |  | Lookup → pum_rbs; write `pum_AdditionalRBSLookup@odata.bind` = `/pum_rbses({id})` |
| `_pum_resourcelookup_value` | Lookup |  | Lookup → pum_resource; write `pum_ResourceLookup@odata.bind` = `/pum_resources({id})` |
| `statuscode` | Status |  |  |
