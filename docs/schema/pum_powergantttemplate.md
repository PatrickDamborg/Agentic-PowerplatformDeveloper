<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_powergantttemplate`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_powergantttemplate -->

# PumPowergantttemplate (`pum_powergantttemplate`)

Read-shape from the Web API: lookups appear as `_<name>_value`; write them via `@odata.bind`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `pum_powergantttemplatename` | String | ✓ | **NAME**; max length 100 |
| `_ownerid_value` | Owner | ✓ | Lookup → systemuser, team; write `OwnerId@odata.bind` = `/systemusers({id})`, `/teams({id})` |
| `pum_powergantttemplateid` | Uniqueidentifier | ✓ |  |
| `statecode` | State | ✓ |  |
...
| `pum_description` | Memo |  | max length 100000 |
| `pum_order` | Integer |  | range -2147483648..2147483647 |
| `pum_shortdescription` | String |  | max length 100 |
| `pum_templateimageid` | Uniqueidentifier |  |  |
