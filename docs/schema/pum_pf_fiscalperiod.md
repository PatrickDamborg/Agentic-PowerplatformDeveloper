<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_pf_fiscalperiod`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_pf_fiscalperiod -->

# PumPfFiscalperiod (`pum_pf_fiscalperiod`)

Read-shape from the Web API: lookups appear as `_<name>_value`; write them via `@odata.bind`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `pum_pf_fiscalperiod` | String |  | **NAME**; max length 100 |
| `_ownerid_value` | Owner | ✓ | Lookup → systemuser, team; write `OwnerId@odata.bind` = `/systemusers({id})`, `/teams({id})` |
| `pum_pf_fiscalperiodid` | Uniqueidentifier | ✓ |  |
| `statecode` | State | ✓ |  |
...
| `pum_pf_actualyear` | Integer |  | range -2147483648..2147483647 |
| `pum_pf_fiscalmonth` | Integer |  | range -2147483648..2147483647 |
| `pum_pf_fiscalperioddefinition` | String |  | max length 100 |
| `pum_pf_fiscalyear` | Integer |  | range -2147483648..2147483647 |
| `pum_pf_fiscalyearprefix` | String |  | max length 100 |
| `statuscode` | Status |  |  |
