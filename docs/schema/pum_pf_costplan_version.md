<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_pf_costplan_version`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_pf_costplan_version -->

# PumPfCostplanVersion (`pum_pf_costplan_version`)

Read-shape from the Web API: lookups appear as `_<name>_value`; write them via `@odata.bind`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `pum_pf_costplan_version` | String | ✓ | **NAME**; max length 100 |
| `_ownerid_value` | Owner | ✓ | Lookup → systemuser, team; write `OwnerId@odata.bind` = `/systemusers({id})`, `/teams({id})` |
| `pum_pf_costplan_version_date` | DateTime (date-only) | ✓ | date (ISO 8601) |
| `pum_pf_costplan_version_description` | String | ✓ | max length 100 |
| `pum_pf_costplan_versionid` | Uniqueidentifier | ✓ |  |
| `statecode` | State | ✓ |  |
...
| `pum_pf_costplan_version_approvaldate` | DateTime (date-only) |  | date (ISO 8601) |
| `pum_pf_costplan_version_approvalstatus` | String |  | max length 100 |
| `pum_pf_costplan_version_default` | Boolean |  |  |
| `_pum_pf_initiative_cpv_value` | Lookup |  | Lookup → pum_initiative; write `pum_pf_initiative_cpv@odata.bind` = `/pum_initiatives({id})` |
