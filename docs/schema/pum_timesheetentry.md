<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_timesheetentry`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_timesheetentry -->

# PumTimesheetentry (`pum_timesheetentry`)
...
| `pum_timesheetentryid` | Uniqueidentifier | ✓ |  |
| `statecode` | State | ✓ |  |
...
| `_pum_assignment_value` | Lookup |  | Lookup → pum_assignment; write `pum_Assignment@odata.bind` = `/pum_assignments({id})` |
| `pum_date` | DateTime (date-only) |  | date (ISO 8601) |
| `pum_daycomment` | Memo |  | max length 4000 |
| `_pum_gantttask_value` | Lookup |  | Lookup → pum_gantttask; write `pum_GanttTask@odata.bind` = `/pum_gantttasks({id})` |
| `pum_hours` | Decimal |  | range 0..24; 1 decimals |
| `_pum_initiative_value` | Lookup |  | Lookup → pum_initiative; write `pum_Initiative@odata.bind` = `/pum_initiatives({id})` |
| `pum_notes` | Memo |  | max length 4000 |
...
| `_pum_timesheet_value` | Lookup |  | Lookup → pum_timesheet; write `pum_Timesheet@odata.bind` = `/pum_timesheets({id})` |
| `statuscode` | Status |  |  |
