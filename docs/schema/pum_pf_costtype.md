<!-- GENERATED — do not edit by hand.
     Source: live EntityDefinitions for `pum_pf_costtype`
     Env:    https://esben.crm.dynamics.com
     Date:   2026-06-16
     Regenerate: uv run python cli.py gen-schema pum_pf_costtype -->

# PumPfCosttype (`pum_pf_costtype`)

Read-shape from the Web API: lookups appear as `_<name>_value`; write them via `@odata.bind`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `pum_pf_costtype_costtype` | String | ✓ | **NAME**; max length 100 |
| `_ownerid_value` | Owner | ✓ | Lookup → systemuser, team; write `OwnerId@odata.bind` = `/systemusers({id})`, `/teams({id})` |
| `pum_pf_abbreviation` | String | ✓ | max length 100 |
| `pum_pf_costtypeid` | Uniqueidentifier | ✓ |  |
| `statecode` | State | ✓ |  |
...
| `pum_levelofdataentry` | Picklist |  |  |
| `pum_pf_costareadef` | Integer |  | range 0..100000 |
| `pum_pf_costcategorydef` | Integer |  | range 0..100000 |
| `pum_pf_costtype_order` | Integer |  | range -2147483648..2147483647 |
| `pum_pf_costtypedef` | Integer |  | range 0..100000 |
| `statuscode` | Status |  |  |
...
### `pum_levelofdataentry` choices

| Value | Label |
|---|---|
| 493840000 | Cost Category |
| 493840001 | Lowest Level (Dynamic) |
| 493840002 | Cost Specification |
